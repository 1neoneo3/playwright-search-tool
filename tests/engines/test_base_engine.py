...
class TestBaseSearchEngine:
    ...
    async def test_context_manager(self, mock_playwright):
        ...
        with patch('src.playwright_search.core.base.async_playwright') as mock_async_playwright:
    ...
    async def test_extract_text_content(self, mock_playwright):
        ...
        mock_playwright['page'].evaluate = AsyncMock(return_value="Removed noise elements")
    ...
    async def test_extract_text_content_fallback(self, mock_playwright):
        ...
        mock_playwright['page'].goto = AsyncMock()
        mock_playwright['page'].evaluate = AsyncMock(side_effect=[
            None,  # Noise removal
            "Fallback body content"  # Body fallback
        ])
        mock_playwright['page'].wait_for_selector = AsyncMock(side_effect=Exception("No main content"))
