...
class SearchPlanGenerator:
    ...
    def create_plan(
        ...
        return SearchPlan(topic=topic, tasks=tasks, created_at=time.time())
...
class ParallelSearchEngine:
    ...
    def __init__(
        self, max_concurrent: int = 5, config: Optional[SearchEngineConfig] = None
    ):
        """
    ...
    async def execute_plan(self, plan: SearchPlan) -> ParallelSearchResult:
        ...
        async def execute_single_task(
            task: SearchTask,
        ) -> Tuple[str, Optional[List[SearchResult]], Optional[str]]:
                    ...
                    async with engine_class(config=self.config) as engine:
                        ...
                        if task.extract_content and search_results:
                            for i, result in enumerate(
                                ...
                                try:
                                    content = await engine.extract_text_content(
                                        result.url
                                    )
                                    if content:
                                    ...
                                    logger.warning(
                                        f"Failed to extract content from {result.url}: {e}"
                                    )
    ...
    def merge_and_deduplicate_results(
        self, parallel_result: ParallelSearchResult
    ) -> List[SearchResult]:
        """
    ...
    def generate_search_summary(
        ...
        return {
            ...
            "total_results": sum(
                len(results) for results in parallel_result.results.values()
            ),
            ...
            "keywords_used": list(
                set(task.keyword for task in parallel_result.plan.tasks)
            ),
            "engines_used": list(
                set(task.engine for task in parallel_result.plan.tasks)
            ),
        }
