...
from ..const import (
    ENGINE_ALIASES, MIN_TIMEOUT, MAX_TIMEOUT, 
    MIN_MAX_CONCURRENT, MAX_MAX_CONCURRENT,
    MAX_NUM_RESULTS, MAX_RECENT_MONTHS,
    ERROR_MESSAGES
)
...
class InputValidator:
    ...
    def validate_concurrent_limit(max_concurrent: int) -> int:
        ...
        if (not isinstance(max_concurrent, int) or 
            max_concurrent < MIN_MAX_CONCURRENT or 
            max_concurrent > MAX_MAX_CONCURRENT):
            raise ValidationError(
    ...
    def validate_num_results(num_results: int) -> int:
        ...
        """
        if not isinstance(num_results, int) or num_results <= 0 or num_results > MAX_NUM_RESULTS:
            raise ValidationError(f"Number of results must be between 1 and {MAX_NUM_RESULTS}")
    ...
    def validate_plan_type(plan_type: str) -> str:
        ...
        if plan_type not in valid_types:
            raise ValidationError(f"Invalid plan type. Valid types: {', '.join(valid_types)}")
