...
class GoogleEngine(BaseSearchEngine):
    ...
    async def search(self, query: str, num_results: int = 10) -> List[SearchResult]:
        ...
        try:
            ...
            search_containers = await self.page.query_selector_all(
                "#search div[data-ved], #search .g, #search [jscontroller], .MjjYud"
            )
            position = 1
            ...
            for element in search_containers:
                ...
                try:
                    ...
                    for existing_result in results:
                        if (
                            existing_result.url == href
                            and existing_result.title == title.strip()
                        ):
                            is_duplicate = True
                            break
                    ...
                    if not is_duplicate:
                        ...
                        recency_score = DateParser.calculate_recency_score(
                            extracted_date
                        )
